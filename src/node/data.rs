...
fn update_last_block(n: u32) -> CleytoResult<()> {
    std::fs::write(ConfigPaths::get().last_block, (n).to_string())?;
    Ok(())
}
...
pub fn write_block(block: &Block) -> CleytoResult<()> {
    ...
    let serialized_block = serde_json::to_string(&block)
        .map_err(CleytonError::BlockSerializationError)?;
    ...
    // update last_block
    update_last_block(last_block + 1)?;

    Ok(())
}

pub fn read_block_by_hash(hash: &String) -> CleytoResult<Block> {
    let block_paths = std::fs::read_dir(PathBuf::from(format!(
        "{}/",
        ConfigPaths::get().block_dir,
    )))
    .unwrap();
    ...
    let regex =
        regex::Regex::new(format!("block_.*_{hash}\\.blk").as_str()).unwrap();

    let block_path = if let Some(v) =
        block_paths.map(|v| v.unwrap()).find(|file| {
    ...
    let serialized_block = std::fs::read_to_string(block_path.path())
        .map_err(|_| CleytonError::BlockNotFound)?;

    serde_json::from_str(&serialized_block)
        .map_err(CleytonError::BlockDeserializationError)
}
...
pub fn read_block_by_number(block_number: &u32) -> CleytoResult<Block> {
    ...
    let regex =
        regex::Regex::new(format!("block_{block_number}_.*\\.blk").as_str())
            .unwrap();
...
pub fn remove_block_by_hash(hash: String) -> CleytoResult<()> {
    ...
    let regex =
        regex::Regex::new(format!("block_(\\d+)_{hash}\\.blk").as_str())
            .unwrap();
    ...
    let (block_path, block_number) = if let Some(v) =
        block_paths.map(|v| v.unwrap()).find_map(|file| {
            ...
            if let Some(caps) = caps_opt {
                println!("Capture [0] is {}", &caps[1]);
                let blk_number: u32 = str::parse(&caps[1]).unwrap();
    ...
    if block_number == Config::get().last_block() {
        ...
        update_last_block(Config::get().last_block() - 1)?;
    }
